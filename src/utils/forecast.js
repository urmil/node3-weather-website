...
const forecast = (longitude,latitude,callback)=>{
    ...
    request({url,json:true},(error,{body})=>{
        if(error){
           ...
           callback(undefined,'Weather is '+body.current.weather_descriptions[0]+'. Temperature is '+ body.current.temperature+' degree out but it feels like '+ body.current.feelslike + ' degree. Humidity is '+body.current.humidity+ ' and Wind speed is around '+body.current.wind_speed+'km/hr to '+body.current.wind_dir + '. Current UV_Index ' + body.current.uv_index  )
        }
